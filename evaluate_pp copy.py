...
for K_dd in [0.3, 0.5, 0.7, 1.0]:
    for alpha in [0.05, 0.1, 0.2, 0.3]:
        ...
        for seed in seeds:
            ...
            if sim is None or sim.result is None:
                print(f"  K_dd={K_dd} alpha={alpha} seed={seed} REJECTED (sim is None)")
                continue

            # Accept scenarioComplete and simulationComplete as valid:
            from scenic.core.simulators import TerminationType
            valid_endings = {
                TerminationType.scenarioComplete,
                TerminationType.timeLimit,
            }
            if sim.result.terminationType not in valid_endings:
                print(f"  K_dd={K_dd} alpha={alpha} seed={seed}  REJECTED reason={sim.result.terminationType}")
                continue
